...
class Controller:
    def __init__(self, quad: Quadrotor3D, t_horizon=1, n_nodes=2000,
                 ...
                 model_name="quad", solver_options=None):
        """
        ...
        q_cost = np.array([8, 8, 80, 6, 0, 0.1, 0.1, 0.1, 5, 5, 0.1])
        ...
        self.quad_xdot = {
            dyn_model_idx: cs.Function('x_dot', [self.x, self.u], [dyn], ['x', 'u'], ['x_dot'])
            for dyn_model_idx, dyn in nominal_with_gp.items()
        }
        ...
        for key, key_model in zip(acados_models.keys(), acados_models.values()):
            ...
            Q = np.diag(np.concatenate((q_diagonal, r_cost)))
            Q_e = np.diag(q_diagonal)
            ...
            # Input constraints
            ocp.constraints.lbu = np.array([self.min_u, self.min_u, 0, self.min_u])
            ocp.constraints.ubu = np.array([self.max_u, self.max_u, 0, self.max_u])
    ...
    def run_optimization(self, initial_state=None, goal=None, use_model=0, return_x=False, mode='pose'):
        ...
        # Set final condition
        if mode == "pose": # for move to goal
            for j in range(self.N):
                y_ref = np.array([goal[0], goal[1], goal[2], 1,0,0,0, 0,0,0, 0,0,2.13481717e+01, 0,0,0,0])
                self.acados_ocp_solver[use_model].set(j, 'p', y_ref)
            y_refN = np.array([goal[0], goal[1], goal[2], 1,0,0,0, 0,0,0, 0,0,2.13481717e+01,0,0,0,0])
            self.acados_ocp_solver[use_model].set(self.N, 'p', y_refN)
        else:   # for tracking trajectory
            for j in range(self.N):
                y_ref = np.array([goal[j,0], goal[j,1], goal[j,2], 1,0,0,0, 0,0,0, 0,0,2.13481717e+01, 0,0,0,0])
                self.acados_ocp_solver[use_model].set(j, 'p', y_ref) 
            y_refN = np.array([goal[self.N,0], goal[self.N,1], goal[self.N,2], 1,0,0,0, 0,0,0, 0,0,2.13481717e+01,0,0,0,0])
            self.acados_ocp_solver[use_model].set(self.N, 'p', y_refN)
